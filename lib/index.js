...
const updatePackageMetaData = async (keys, packageVersion_id, uploadData) => {
    return new Promise(async (resolve, reject) => {
        try {
            var config = { headers: { Authorization: `Bearer ${keys.api_token}` }}
            var res = await axios.put(`${this.host}/store-publishing/package-version/${packageVersion_id}`,
            {
                'versionName': uploadData.versionName,
                'price': uploadData.price,
                'category': uploadData.category,
                'metadatas': {
                  'en_US': uploadData.metadatas?.en_US,
                  'zh_CN': uploadData.metadatas?.zh_CN,
                  'ko_KR': uploadData.metadatas?.ko_KR,
                  'ja_JP': uploadData.metadatas?.ja_JP
                },
...
const updatePackageKeyImagesData = async (keys, packageVersion_id, uploadData) => {
    return new Promise(async (resolve, reject) => {
        try {
            var config = { headers: { Authorization: `Bearer ${keys.api_token}` }}
            for (const [keyImageType, source] of Object.entries(uploadData.keyImages || {})) {
            
                const url = `${this.host}/store-publishing/package-version/${packageVersion_id}/keyimage/${keyImageType}`;
                const form = new FormData();
...
const updatePackageSRPData = async (keys, packageVersion_id, packageVersionData, uploadData) => {
    return new Promise(async (resolve, reject) => {
        try {
            packageVersionData.unitypackages[uploadData.unity_version].srps = uploadData.srps;
            packageVersionData.unitypackages[uploadData.unity_version].dependencies = [];
            var config = { headers: { Authorization: `Bearer ${keys.api_token}` }}
            var res = await axios.put(`${this.host}/store-publishing/package-version/${packageVersion_id}`,
            {
                'unitypackages': packageVersionData.unitypackages,
            }, config);
            resolve(res.data);
        } catch (err) {
            console.log(err)
...
module.exports = {
    ...
    updatePackageKeyImagesData,
    updatePackageSRPData
};